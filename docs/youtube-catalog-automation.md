...
The workflow is manual or daily. It reads the `YOUTUBE_API_KEY` Actions secret, never serializes it, uploads diagnostics, and only deploys Pages when the repository variable is enabled. The repository owner/name must be taken from `git remote -v`; no URL is hard-coded here. The app uses the HTTPS Pages URL by default (`https://dzndorosh.github.io/doomscrolling/catalog/youtube-catalog.json`); `FOCUSREELS_REMOTE_CATALOG_URL` is an optional override for development and forks. Runtime accepts only HTTPS, rejects redirects, limits response size, and falls back to cache/bundled fixture.
